...
import qs from 'qs';
import { fetchStorybookPreviewResource, evalStorybookEnvironmentInfo, evalStorybookStorySnapshots, evalSetCurrentStory, validateStoryArgs, encodeStoryArgs, withPage } from './utils.js'; // Returns true or false if the provided story should be skipped by matching against include and
...
function shouldSkipStory(name, options, config) {
  let matches = regexp => {
    ...
    return (_regexp = regexp) === null || _regexp === void 0 ? void 0 : (_regexp$test = _regexp.test) === null || _regexp$test === void 0 ? void 0 : _regexp$test.call(_regexp, name);
  }; // if a global filter is present, disregard story filters


  let filter = config !== null && config !== void 0 && config.include || config !== null && config !== void 0 && config.exclude ? config : options;
  let include = [].concat(filter === null || filter === void 0 ? void 0 : filter.include).filter(Boolean);
  let exclude = [].concat(filter === null || filter === void 0 ? void 0 : filter.exclude).filter(Boolean); // if included, don't skip; if excluded always exclude

  let skip = include !== null && include !== void 0 && include.length ? !include.some(matches) : options.skip;
  if (!skip && !(exclude !== null && exclude !== void 0 && exclude.some(matches))) return false;
  return true;
...
function mapStorybookSnapshots(stories, {
  ...
  if (!snapshots.length) throw new Error('No snapshots found'); // remove filter options and generate story snapshot URLs
...
export async function* takeStorybookSnapshots(percy, callback, {
  ...
  try {
    ...
    let lastCount;
    log.debug(`Requesting Storybook: ${baseUrl}`); // start a timeout to show a log if storybook takes a few seconds to respond
    ...
    clearTimeout(logTimeout); // start percy

    yield* percy.yield.start(); // launch the percy browser if not launched during dry-runs

    yield percy.browser.launch(); // gather storybook data in parallel

    let [environmentInfo, stories] = yield* yieldAll([withPage(percy, aboutUrl, p => p.eval(evalStorybookEnvironmentInfo)), withPage(percy, previewUrl, p => p.eval(evalStorybookStorySnapshots))]); // map stories to snapshot options
    ...
    percy.client.addEnvironmentInfo(environmentInfo); // use a single page to capture story snapshots without reloading
    ...
    yield* withPage(percy, previewUrl, async function* (page) {
      ...
      while (snapshots.length) {
        ...
        PercyConfig.validate(options, '/snapshot/dom'); // snapshots are queued and do not need to be awaited on

        percy.snapshot(options); // discard this story snapshot when done

        snapshots.shift();
      }
    }, () => {
      log.debug(`Page crashed while loading story: ${snapshots[0].name}`); // return true to retry as long as the length decreases
