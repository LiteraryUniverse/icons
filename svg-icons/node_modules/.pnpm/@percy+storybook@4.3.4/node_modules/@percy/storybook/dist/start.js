...
export const start = command('start', {
  ...
  examples: ['$0', '$0 --port 9000', '$0 --static-dir public'],
  percy: {
    delayUploads: true
  }
}, async function* ({
  ...
  let proc = yield new Promise((resolve, reject) => resolve(spawn('start-storybook', args, {
    stdio: 'inherit'
  }).on('error', reject)));
  /* istanbul ignore next: this is a storybook flag we don't need to test */

  let baseUrl = `${argv.includes('--https') ? 'https' : 'http'}://${host}:${port}`;
  yield* takeStorybookSnapshots(percy, () => proc.kill(), {
    baseUrl,
    flags
  });
});
