import { request, createRootResource, yieldTo } from '@percy/cli-command/utils'; // Transforms authorization credentials into a basic auth header and returns all config request
...
export async function* withPage(percy, url, callback, retry) {
  ...
  try {
    ...
    if ((_error$message = error.message) !== null && _error$message !== void 0 && _error$message.includes('crashed') && retry !== null && retry !== void 0 && retry()) {
      return yield* withPage(...arguments);
    }
    /* istanbul ignore next: purposefully not handling real errors */


    throw typeof error !== 'string' ? error : new Error(error.replace( // strip generic error names and confusing stack traces
...
export function evalSetCurrentStory({
  ...
  return waitFor(() => {
    ...
    let {
      ...
      globals,
      args
    } = story; // emit a series of events to render the desired story
